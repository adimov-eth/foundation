...
Canonical one-line definitions. Notes link here as `[[glossary#<slug>]]` where `<slug>` is the
lowercase-kebab heading below — never redefine a term inline. Each entry points to the note that
develops it. Headings are the stable anchor contract (validated by `docs/_index/check.py`).
