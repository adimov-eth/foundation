...
export abstract class ToolInteraction<T extends Record<string, any>> {
  /**
   * Override this static property to provide a custom tool name.
   * If not provided, defaults to the class name.
   */
  static readonly toolName?: string;
  
  abstract readonly description: string | Promise<string>;

  constructor(public readonly context: Context) {}

  async getToolDescription(): Promise<Tool> {
    const ToolClass = this.constructor as typeof ToolInteraction & { toolName?: string };
    return {
